...
name: opencode-agent
description: >
  Delegate tasks to an opencode agent that uses alternative AI models (default: glm-5-turbo).
...
Delegate tasks to [opencode](https://opencode.ai), an alternative AI coding agent. This skill runs `opencode run` as a subprocess, letting you hand off work to models like glm-5-turbo without leaving your Claude Code session.
...
Run the user's task via `opencode run`, passing the task as the message argument. The default model is **glm-5-turbo**. If the user specifies a different model, pass it with the `-m` flag.
...
1. **Parse the request.** Extract the task the user wants opencode to perform. If the user specified a model, note it. Otherwise default to glm-5-turbo.
...
2. **Run opencode.** Execute the task using Bash. Choose the right execution strategy based on the task's expected duration:
   ...
   **Medium tasks** (multi-file changes, test generation, refactoring) — use a longer timeout up to 10 minutes:
   ```bash
   opencode run "<task>"   # with timeout set to 600000
   ```
   ...
   **Long-running tasks** (large codebases, complex generation, full project scaffolding) — run in the background so the user isn't blocked:
   ```bash
   opencode run "<task>"   # with run_in_background: true
   ```
   ...
   If the user specified a model, add `-m <model>`. Otherwise default to glm-5-turbo.
